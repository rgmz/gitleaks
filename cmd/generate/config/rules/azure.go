...
func AzureActiveDirectoryClientSecret() *config.Rule {
	...
	r := config.Rule{
		...
		Regex:       regexp.MustCompile(`(?:^|[\\'"` + "`" + ` >=:\(,)])([a-zA-Z0-9_~.]{3}\dQ~[a-zA-Z0-9_~.-]{31,34})(?:$|[\\'"` + "`" + ` <\),])`), // wtf, Go? https://github.com/golang/go/issues/18221
		SecretGroup: 1,
		Entropy:     1,
		Keywords: []string{
			"Q~",
	...
	tps := []string{
		...
		`(QtT8Q~9C-_Ij~RouHVpD2Tuf3oHWGh.DQ3kcjbAn)`,                                                                                                          // gitleaks:allow
		`\"pass\": \"` + fmt.Sprintf("%s%sQ~%s", secrets.NewSecret(`[\w~.]{3}`), secrets.NewSecret(numeric("1")), secrets.NewSecret(`[\w~.-]{31,34}`)),
	}
	fps := []string{
