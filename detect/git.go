				...
				d.Sema.Go(func() error {
					for _, textFragment := range gitdiffFile.TextFragments {
						...
						for _, finding := range d.Detect(fragment) {
							d.addFinding(augmentGitFinding(remote.Platform, remote.Url, finding, gitdiffFile, textFragment))
						}
					}
					return nil
				})
...
func (d *Detector) detectBinary(repo *git.Repository, remote *RemoteInfo, commitHash string, gitdiffFile *gitdiff.File) error {
	...
	for {
		...
		if n > 0 {
			...
			for _, finding := range d.Detect(fragment) {
				d.addFinding(augmentGitFinding(remote.Platform, remote.Url, finding, gitdiffFile, nil))
			}
		}
